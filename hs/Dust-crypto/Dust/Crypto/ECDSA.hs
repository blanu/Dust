...
createSigningKeypair = do
    (Ed.PublicKey pub, Ed.SecretKey priv) <- Ed.createKeypair
    return $ Keypair (PublicKey pub) (PrivateKey priv)

sign :: ByteString -> PrivateKey -> Signature
sign bs (PrivateKey private) =
    let (Ed.Signature sig) = Ed.sign' (Ed.SecretKey private) bs
    in Signature sig

verify :: ByteString -> Signature -> PublicKey -> Bool
verify bs (Signature sig) (PublicKey public) = Ed.verify' (Ed.PublicKey public) bs (Ed.Signature sig)
