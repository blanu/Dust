...
HEADER_SIZE=IV_SIZE+MAC_SIZE+TIMESTAMP_SIZE+DATA_LENGTH_SIZE
...
class DustPacket:
  ...
  def createDustPacket(self, key, data, entropy):
    ...
    payloadLength=TIMESTAMP_SIZE+DATA_LENGTH_SIZE+len(self.data)

    self.timestamp=getTime()
    timestamp=makeTimestamp(self.timestamp)
    self.dataLength=len(self.data)
    dataLength=makeLength(self.dataLength, DATA_LENGTH_SIZE)
    self.payload=timestamp+dataLength+self.data
    ...
    self.packet=self.mac+self.ciphertext
  ...
  def decodeDustPacket(self, key, packet):
    ...
    self.timestamp, self.dataLength, self.data=splitFields(self.payload, [TIMESTAMP_SIZE, DATA_LENGTH_SIZE])
    ...
    realPacketLength=MAC_SIZE+ciphertextLength
